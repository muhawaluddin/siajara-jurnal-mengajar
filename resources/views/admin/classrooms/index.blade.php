...
@section('page-subtitle', 'Kelola daftar kelas yang digunakan oleh siswa.')
...
<div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
    <form method="GET" action="{{ route('admin.classrooms.index') }}" class="flex items-center gap-3">
...
<div class="overflow-x-auto rounded-xl border border-slate-200">
    <table class="min-w-full divide-y divide-slate-200 text-sm">
        ...
        @forelse($classrooms as $classroom)
            <tr>
                <td class="px-4 py-3 font-medium text-slate-800">{{ $classroom->name }}</td>
                <td class="px-4 py-3 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('admin.classrooms.edit', $classroom) }}" class="rounded-lg border border-emerald-200 px-3 py-1 text-xs font-semibold text-emerald-600 hover:bg-emerald-50">Ubah</a>
        ...
        @empty
            <tr>
                <td colspan="2" class="px-4 py-6 text-center text-sm text-slate-500">Belum ada data kelas.</td>
            </tr>
        @endforelse
        </tbody>
    </table>
